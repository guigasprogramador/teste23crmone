...
DROP FUNCTION IF EXISTS crmonefactory.api_get_orgao_contatos;
DROP FUNCTION IF EXISTS crmonefactory.api_create_orgao_contato;
DROP FUNCTION IF EXISTS crmonefactory.api_update_orgao_contato;
DROP FUNCTION IF EXISTS crmonefactory.api_delete_orgao_contato;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE routine_schema = 'crmonefactory' 
    AND routine_name = 'api_get_orgao_contatos'
  ) THEN
    RAISE EXCEPTION 'Função api_get_orgao_contatos não foi criada';
  END IF;
END $$;
